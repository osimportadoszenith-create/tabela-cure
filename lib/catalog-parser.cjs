...
function normalizeProduct(product) {
  ...
  return {
    ...
    displayBrand: String(product.displayBrand || product.brand || "").trim(),
...
function normalizeBrand(brand) {
  const imageData = String(brand.imageData || "").trim();
  return {
    id: String(brand.id || brand.name || "").trim(),
    name: String(brand.name || "").trim(),
    imageData: /^(?:https?:\/\/|\/)/i.test(imageData) ? imageData : "",
    imagePosition: normalizeOptionalText(brand.imagePosition),
    updatedAt: normalizeOptionalText(brand.updatedAt),
  };
}

function parseSourceBrands(value) {
  let rawBrands = value;
  if (typeof value === "string") {
    try {
      rawBrands = JSON.parse(value);
    } catch (error) {
      throw new Error(`A fonte não contém uma lista de marcas válida: ${error.message}`);
    }
  }
  if (!Array.isArray(rawBrands)) throw new Error("A fonte não contém uma lista de marcas válida.");
  const brands = rawBrands
    .filter((brand) => !brand.isDeleted && brand.isActive !== false && brand.name)
    .map(normalizeBrand);
  if (!brands.length) throw new Error("A fonte retornou uma lista de marcas vazia.");
  return brands;
}
